...
our $lcov_version	= "LCOV version 1.15.alpha0w";
...
our $os_is_windows;
our $dir_sep;        # Directory separator character
...
$os_is_windows = is_os_type('Windows');
...
elsif (($os_is_windows) and (-r "$tool_dir/../etc/lcovrc"))
{
	$config = read_config("$tool_dir/../etc/lcovrc");
}
elsif (($os_is_windows) and (-r "$tool_dir/../lcovrc"))
{
    $config = read_config("$tool_dir/../lcovrc");
}
...
if( $os_is_windows )
{
	...
	if( defined($desc_filename) ) { $desc_filename =~ s/\//\\/g; }
	if( defined($css_filename) ) { $css_filename =~ s/\//\\/g; }
	if( defined($base_filename) ) { $base_filename =~ s/\//\\/g; }
	for( @opt_dir_prefix )
	{
		s/\//\\/g;
	}
	if( defined($opt_config_file) ) { $opt_config_file =~ s/\//\\/g; }
	for( @info_filenames )
	{
		s/\//\\/g;
	}
...
if ($css_filename)
{
	$css_filename = resolve_absolute_path($css_filename, $cwd);
}
...
#
# Resolves the absolute path of PATH, prefixig CWD if PATH is not absolute
#
...
sub gen_html()
{
	...
	else
	{
		...
		{
				$dir_prefix[$i] = resolve_absolute_path( $dir_prefix[$i], $cwd );
				$msg .= ", " unless 0 == $i;
				$msg .= "\"" . $dir_prefix[$i] . "\"";
		}
		info($msg . "\n");
...
sub write_dir_page($$$$$$$$$$$$$$$$$)
{
	...
	html_create(*HTML_HANDLE, $rel_dir.$dir_sep."index$name.$html_ext");
...
sub process_dir($)
{
	...
	if( $os_is_windows )
	{
		# Change volume name to be "directory friendly"
		$rel_dir =~ s/^([a-zA-Z]):/$1/;
	}
	else
	{
...
sub write_function_page($$$$$$$$$$$$$$$$$$)
{
	...
	if ($sort_type == 0) {
		$filename = "$rel_dir$dir_sep$base_name.func.$html_ext";
	} else {
		$filename = "$rel_dir$dir_sep$base_name.func-sort-c.$html_ext";
	}
	html_create(*HTML_HANDLE, $filename);
	$pagetitle = "LCOV - $title - $trunc_dir$dir_sep$base_name - functions";
	write_html_prolog(*HTML_HANDLE, $base_dir, $pagetitle);
	write_header(*HTML_HANDLE, 4, "$trunc_dir$dir_sep$base_name",
		     "$rel_dir$dir_sep$base_name", $lines_found, $lines_hit,
...
sub process_file($$$)
{
	...
	html_create(*HTML_HANDLE, "$rel_dir$dir_sep$base_name.gcov.$html_ext");
	$pagetitle = "LCOV - $test_title - $trunc_dir$dir_sep$base_name";
	write_html_prolog(*HTML_HANDLE, $base_dir, $pagetitle);
	write_header(*HTML_HANDLE, 2, "$trunc_dir$dir_sep$base_name",
		     "$rel_dir$dir_sep$base_name", $lines_found, $lines_hit,
	...
	gen_png("$rel_dir$dir_sep$base_name.gcov.png", $overview_width, $tab_size,
		@source);

	# Create frameset page
	html_create(*HTML_HANDLE,
		    "$rel_dir$dir_sep$base_name.gcov.frameset.$html_ext");
	write_frameset(*HTML_HANDLE, $base_dir, $base_name, $pagetitle);
	close(*HTML_HANDLE);

	# Write overview frame
	html_create(*HTML_HANDLE,
		    "$rel_dir$dir_sep$base_name.gcov.overview.$html_ext");
...
sub shorten_prefix($)
{
	my @list = split("\\".$dir_sep, $_[0]);

	pop(@list);
	return join($dir_sep, @list);
}
	...
	elsif ($type == $HDR_SOURCE || $type == $HDR_FUNC)
	{
		...
		if ($frames)
		{
			...
			$view = "<a href=\"$base_dir"."index.$html_ext\" ".
				...
				"$esc_trunc_name</a> - $esc_base_name";
		}
		else
		{
			$view = "<a href=\"$base_dir"."index.$html_ext\">".
...
sub apply_prefix($@)
{
	...
	if (@dir_prefix)
	{
		...
		{
			if ($prefix ne "" && $filename =~ /^\Q$prefix$dir_sep\E/)
			{
				return substr($filename, length($prefix.$dir_sep));
			}
		}
	}
