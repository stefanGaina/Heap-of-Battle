...
if( $os_is_windows )
{
	...
	if( defined($list) ) { $list =~ s/\//\\/g; }
	if( defined($extract) ) { $extract =~ s/\//\\/g; }
	if( defined($remove) ) { $remove =~ s/\//\\/g; }
	if( defined($diff_path) ) { $diff_path =~ s/\//\\/g; }
	if( defined($output_filename) ) { $output_filename =~ s/\//\\/g; }
	if( defined($base_directory) ) { $base_directory =~ s/\//\\/g; }
	if( defined($gcov_tool) ) { $gcov_tool =~ s/\//\\/g; }
	if( defined($to_package) ) { $to_package =~ s/\//\\/g; }
	if( defined($from_package) ) { $from_package =~ s/\//\\/g; }
...
sub userspace_reset()
{
	...
	foreach $current_dir (@directory)
	{
		...
		if ($os_is_windows)
		{
			@file_list = `dir /s/b "$current_dir\\*.da" 2>NUL`;
			push( @file_list, `dir /s/b "$current_dir\\*.gcda" 2>NUL` );
		}
		else
		{
...
sub lcov_geninfo(@)
{
	...
	@param = ("perl", $tool_dir.$dir_sep."geninfo", @dir);
	if ($output_filename)
	{
...
sub list()
{
	...
	foreach $filename (sort(keys(%{$data})))
	{
		...
		if (!$opt_list_full_path) {
			...
			if (!$got_prefix || !$root_prefix &&
			    !($print_filename =~ s/^\Q$prefix$dir_sep\E//)) {
				my ($v, $d, $f) = splitpath($filename);
...
sub strip_directories($$)
{
	...
	for ($i = 0; $i < $depth; $i++)
	{
		$filename =~ s/^[^\Q$dir_sep\E]*\Q$dir_sep\E+(.*)$/$1/;
	}
	return $filename;
}
	...
	foreach (keys(%{$diff_data}))
	{
		...
		if ($filename =~ /^\Q$diff_path$sep$_\E$/)
		{
			if ($diff_name)
			{
				...
				$old_depth = ($diff_name =~ tr/$dir_sep//);
				$new_depth = (tr/$dir_sep//);
...
sub convert_paths($$)
{
	...
	foreach $filename (keys(%{$path_conversion_data}))
	{
		...
		while (($filename =~ s/^(.*)\Q$dir_sep\E[^\Q$dir_sep\E]+$/$1/) &&
		       ($new_path =~ s/^(.*)\Q$dir_sep\E[^\Q$dir_sep\E]+$/$1/) &&
