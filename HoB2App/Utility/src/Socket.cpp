...
void Socket::receiveUpdates(Message& updateMessage) const noexcept
{
	...
	receivedBytes = recv(m_clientSocket, reinterpret_cast<char*>(&updateMessage), sizeof(Message), 0L);
	if (0L == receivedBytes)
	{
...
void Socket::closeServer(void) noexcept
{
	int32_t errorCode = NO_ERROR;

	plog_trace("Server socket is being closed.");
	if (INVALID_SOCKET != m_serverSocket)
	{
		errorCode = closesocket(m_serverSocket);
		if (NO_ERROR != errorCode)
		{
			plog_error("Server socket failed to be closed! (error code: %" PRId32 ")", errorCode);
		}
		m_serverSocket = INVALID_SOCKET;
	}
	else
	{
		plog_warn("Server socket is already closed.");
	}
}
